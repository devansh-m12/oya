...
library RandomNumberErrors {
    error IncorrectSender();

    error InsufficientFee();
}

/// Example contract using Pyth Entropy to allow a user to request a secure random number.
...
///
/// The RandomNumber contract implements the IEntropyConsumer interface imported from the Solidity SDK.
...
contract RandomNumber is IEntropyConsumer {
    ...
    // Event emitted when the result of the random number request is known.
    ...
    function requestRandom() external payable {
        ...
        // This implementation of the requestRandom method passes on the fee to the end user.
        uint256 fee = entropy.getFeeV2();
        if (msg.value < fee) {
            revert RandomNumberErrors.InsufficientFee();
        }
        ...
        emit RandomRequest(sequenceNumber);
    }

    // Request a random number with a custom gas limit.
    function requestRandomWithCustomGasLimit(uint32 gasLimit) external payable {
    ...
    // Request a random number with a custom provider and custom gas limit.
    function requestRandomWithCustomProviderAndGasLimit(address provider, uint32 gasLimit) external payable {
    ...
    // Request a random number with a custom provider and custom gas limit and userContribution / Random Number.
    function requestRandomWithCustomProviderAndGasLimitAndUserContribution(address provider, uint32 gasLimit, bytes32 userContribution) external payable {
        uint256 fee = entropy.getFeeV2(provider, gasLimit);
        if (msg.value < fee) {
            revert RandomNumberErrors.InsufficientFee();
        }
    ...
    function entropyCallback(
        ...
        emit RandomResult(sequenceNumber, randomNumber);
    }
    ...
    function getRandomFee() public view returns (uint256) {
